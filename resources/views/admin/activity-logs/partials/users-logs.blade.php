<div id="users-logs" class="tab-content">
            ...
            @foreach($logs as $log)
                <div class="bg-gray-50 rounded-lg p-4 border-l-4 border-purple-500">
                    <div class="flex items-start justify-between">
                            ...
                            <div class="flex items-center space-x-2 mb-2">
                                ...
                                @if($log->target_user_role)
                                    <span class="px-2 py-1 text-xs rounded-full
                                        {{ $log->target_user_role == 'admin' ? 'bg-red-100 text-red-800' : '' }}
                                        {{ $log->target_user_role == 'operator' ? 'bg-yellow-100 text-yellow-800' : '' }}
                                        {{ $log->target_user_role == 'user' ? 'bg-blue-100 text-blue-800' : '' }}">
                                        {{ ucfirst($log->target_user_role) }}
                                    </span>
                                @endif
                                @if($log->password_changed)
                                    <span class="px-2 py-1 text-xs bg-orange-100 text-orange-800 rounded-full">Password Changed</span>
                                @endif
                            </div>
                        ...
                        <div class="ml-4">
                            <button onclick="toggleDetails('users-{{ $log->id }}')"
                    ...
                    <div id="users-{{ $log->id }}" class="hidden mt-4 pt-4 border-t border-gray-200">
    ...
    @else
        <div class="text-center py-12">
            ...
            <h3 class="text-lg font-medium text-gray-900 mb-2">No Users Logs Found</h3>
