@php($items = $locations->where('type', 'wfa'))
@if($items->isEmpty())
    <tr>
        <td colspan="5" class="px-8 py-16 text-center">
            <div class="flex flex-col items-center">
                <div class="w-20 h-20 bg-gradient-to-br from-purple-100 to-purple-200 rounded-full flex items-center justify-center mb-6">
                ...
                <h3 class="text-xl font-bold text-gray-900 mb-2">Belum ada lokasi WFA</h3>
                <p class="text-gray-600 mb-6 max-w-sm">Mulai dengan menambahkan lokasi WFA untuk work from anywhere</p>
                <a href="{{ route('admin.locations.create') }}"
                    class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 text-white font-bold rounded-xl transition-all duration-200 transform   shadow-lg">
...
@else
    @foreach($items as $location)
        <tr class="hover:bg-purple-50 transition-colors duration-200 group">
            <td class="px-4 py-6">
                <input type="checkbox" name="ids[]" value="{{ $location->id }}" class="row-select w-4 h-4 text-purple-600 border-gray-300 rounded focus:ring-purple-500">
            </td>
            <td class="px-8 py-6 whitespace-nowrap">
                <div class="flex items-center">
                    <div class="w-10 h-10 bg-gradient-to-br from-purple-100 to-purple-200 rounded-xl flex items-center justify-center mr-4 group-hover:from-purple-200 group-hover:to-purple-300 transition-colors">
